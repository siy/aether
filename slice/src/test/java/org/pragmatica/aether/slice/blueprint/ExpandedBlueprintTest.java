                ...
                Artifact.artifact("org.example:slice:1.0.0")
                    ...
                    .flatMap(resolved ->
                        Artifact.artifact("org.example:dependency:2.0.0")
                            .map(depArtifact -> ResolvedSlice.resolvedSlice(depArtifact, 1, true))
                            .map(dep -> List.of(dep, resolved))
                            .map(loadOrder -> ExpandedBlueprint.expandedBlueprint(id, loadOrder, List.of()))
                    )
            )
        ...
        BlueprintId.blueprintId("my-app:1.0.0")
            .flatMap(id ->
                RoutingSection.routingSection("http", Option.none(), List.of())
                    ...
                    .map(routing -> ExpandedBlueprint.expandedBlueprint(id, List.of(), routing))
            )
            .onFailureRun(Assertions::fail)
            .onSuccess(expanded -> {
